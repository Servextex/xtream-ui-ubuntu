...
$PACKAGE_INSTALLER libzip-devel
	...
	#install pip2
	wget -qO- https://bootstrap.pypa.io/pip/2.7/get-pip.py | python3 - 'pip==20.3.4'
	#upgrade pip3
...
EOF
	...
	apt-get purge libcurl3 -y
	$PACKAGE_INSTALLER libcurl4 libxslt1-dev libgeoip-dev e2fsprogs wget mcrypt libmcrypt-dev nscd htop unzip ufw apache2 zip mc libpng16-16 python3 python3
	ufw disable
...
wget -q -O /tmp/xtreamcodes.tar.gz https://github.com/Servextex/xtream-ui-ubuntu/releases/download/start/sub_xui_"$OSNAME"_"$VER".tar.gz
